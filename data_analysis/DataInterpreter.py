...
def mainScript(folder, file):
    ...
    timeInput = timeInputRaw.replace(':', '')
    if timeInput == "":
        metadata = file.split(",")
        if len(metadata) >= 4:
...
while loop:
    ...
    if actionInput == "clear":
        inputHelp = False
        with open("data_analysis/utilities/ClearSpreadsheet.py") as f:
            exec(f.read())
    elif actionInput == "new":
        inputHelp = False
        with open("data_analysis/utilities/NewSpreadsheet.py") as f:
            exec(f.read())
